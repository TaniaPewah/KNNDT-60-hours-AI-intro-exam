...
def splits(dataset, n_folds):
    ...
    new_split = list()
    new_split[0] = dataset_split[0]
    new_split[1] = dataset_split[0] + dataset_split[1]
    new_split[2] = dataset_split[0] + dataset_split[1] + dataset_split[2]
    new_split[3] = dataset_split[0] + dataset_split[1] + dataset_split[2] + dataset_split[3]
    new_split[4] = dataset
    return dataset_split

def run_increasing_test_folds( data, test ):
    accuracies = []
    num_of_items = []
    test = test.to_numpy()
    spl = splits(data, 5)
    for i, split in enumerate(spl):
        num_of_items.append(len(split))
        split = np.array(split)
        accuracy = KNN_build_and_test(split, test, 7, 3, 0.01,{})
        accuracies.append(accuracy)
        print(i, ' split accuracy:', accuracy)

    plt.plot( num_of_items, accuracies)
    plt.title('gradual increase in examples Accuracies')

    plt.xlabel('Num og items')
    plt.ylabel('Accuracy')
    plt.show()
...
def splits(dataset, n_folds):
    ...
    for _ in range(n_folds):
        ...
        while len(fold) < fold_size:
            ...
            row = dataset_copy[index]
            dataset_copy = np.delete(dataset_copy, index, 0)
            fold.append(row)
        dataset_split.append(fold)

    new_split = list()
    new_split.append(dataset_split[0])
    new_split.append(dataset_split[0] + dataset_split[1])
    new_split.append(dataset_split[0] + dataset_split[1] + dataset_split[2])
    new_split.append(dataset_split[0] + dataset_split[1] + dataset_split[2] + dataset_split[3])
    new_split.append(dataset)
    return new_split
